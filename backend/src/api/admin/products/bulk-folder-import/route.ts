...
function validateProduct(data: any, rowIndex: number): { valid: boolean; data?: any; errors?: string[] } {
  ...
  // Support both cartonPrice and cartonPcsPrice field names - make optional
  const cartonPriceValue = data.cartonPrice || data.cartonPcsPrice;
  const cartonPrice = cartonPriceValue ? parseFloat(cartonPriceValue) : singlePrice || 0;
  if (cartonPriceValue && (isNaN(cartonPrice) || cartonPrice < 0)) {
    errors.push('Carton price must be a valid positive number');
  }
  ...
  return {
    ...
    data: {
      ...
      cartonPcsPrice: cartonPrice,
      cartonQty: parseInt(data.cartonQty) || 1,
      stock,
...
router.post('/', upload.single('file'), async (req: Request, res: Response) => {
  try {
    ...
    const headers = [
      "productCode", "name", "description", "singlePrice", "cartonPrice", "cartonQty", 
    ...
    for (const product of products) {
      const row: string[] = [
        ...
        "",        // cartonPrice (empty for user to fill) ✅ ADDED
        "",        // cartonQty
        "",        // gstPercentage
        "",        // hsnCode
        "",        // weight
      ...
      return res.status(500).json({ success: false, message: error.message || "Error processing ZIP file" });
    }
  });
